...
| `test_mcp_contract.py` | The ~150-test tool-surface contract, driven in-process (no live transport, no SDK needed): create → batch upsert (per-item verdicts naming violated constraints) → query → trace → `gate_run` → the execution loop (`progress_update` / `audit_record` / `work_bind`) → handoff emission + injection screen → lockfile conflict → `export_html` → the missing-SDK error path → `--selftest`. Includes the V4Engine behavior and the teaching-surface needles (prompts/templates may only teach tool and gate names that exist). |

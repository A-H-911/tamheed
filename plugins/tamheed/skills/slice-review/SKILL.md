...
name: slice-review
description: >-
  Invoke when a slice is believed complete: per-criterion verdicts against the criterion's own exported text, bindings, the closing entry, then readiness_check at slice scope - the review that moves Review to Implemented.
...
Invoke this (`/tamheed:slice-review`) when a slice of `<package>` is believed complete.

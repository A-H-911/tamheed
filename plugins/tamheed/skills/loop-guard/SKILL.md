...
name: loop-guard
description: >-
  Invoke to read the brake for fully-auto execution: the stop conditions an unattended loop evaluates every iteration; scope decisions and forced transitions always need a human.
...
Keep this (`/tamheed:loop-guard`) beside `/tamheed:loop-iteration` when running `<package>` unattended. The loop stops

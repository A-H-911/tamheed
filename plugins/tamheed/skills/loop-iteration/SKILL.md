...
name: loop-iteration
description: >-
  Invoke as the repeated prompt of an unattended loop (an in-session loop or an external harness): one fully-auto pass over one work item, ending with the machine-parseable ITERATION line.
...
`<package>` (an in-session loop or an external harness invoking it per iteration — both parse
the ITERATION block). Read the loop guard first — `${CLAUDE_PLUGIN_ROOT}/skills/loop-guard/SKILL.md` — its stop
conditions override everything here.

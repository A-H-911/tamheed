...
1. `package_open("{package}")`, then `package_verify()` — the canonical round-trip
   ...
   stored trace edges violating the endpoint rules; `relates_to` is the escape
   hatch); treat a G-TRACE "passed vacuously" warning as a finding, not a pass.
2. Spot-check counts: `entity_query("requirement", limit=1)` and read `total`;
   ...
   `data/*.jsonl` to dodge a payload cap.
...
4. Audit honesty: `gate_run`'s `audit_evidence.narrated_ids` names every NARRATED
   verdict (no evidence) — each is the graded party grading itself; read them via
   `entity_query("audit-verdict", ids=[...])`. Sweep for rulings hiding in CLOSED
...
9. Report: verify verdict (+ digest), gate verdict, count anomalies, trace gaps,
   narrated verdicts, buried rulings, staleness, unbound commits, readiness
   blockers, repair-verification mismatches — then `package_close()`. Change
   NOTHING in this run.

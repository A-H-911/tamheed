...
| `handoff_emit(target_dir, subdir?, force?)` | mutate | **Managed emissions**: every file reports as `written`/`unchanged`/`diverged` — a hand-edited file is refused (never silently clobbered) unless `force=true`. Emits prompt files (into `<target>/<subdir>`, default `handoff/`) + executor-side `.mcp.json` (omitted on plugin-hosted servers) + the `CLAUDE.md` operating note; the stale-v1 warning lives in a marker-managed block that RETRACTS itself once the scan is clean (re-run emit = self-contained cutover verifier). Emits the scenario library into `<package>/prompts/`; reports `stale_references` AND `restated_content` (register content restated in CLAUDE.md/AGENTS.md — `unlabeled` gets a reference rewrite, `labeled-snapshot` gets verify-currency guidance); injection-screened |
...
| `export_html(output?)` | export | Render the HTML review surface to `<package>/review.html` — dark maximalist identity, State→Relations→Data section order, sticky TOC, the all-data relations graph (every node links to its row), all tables folded with per-table `csv/<table>.csv` download links (deterministic; `csv/` is DERIVED — hand edits are overwritten on re-export, while authored emissions remain protected), wrap-in-place text, honest freshness |

...
| `gate_run()` | read | Referential gates VERIFY at gate time (plan 027: `PRAGMA foreign_key_check`, entity_index consistency, real status/provenance SELECTs — whitespace-only provenance caught); coverage gates run the SQL views; content tier scans placeholders (code spans stripped per the frozen v1 contract; `custom_attributes` exempt as provenance); warns when G-TRACE passes vacuously (0 MVP rows); audit evidence split evidenced/narrated; advisory `relation_rules` sweep lists legacy mistyped edges (never blocks) |
| `readiness_check(scope?, id?)` | read | Deep lifecycle readiness at a close boundary (package/phase/slice): blocking rules (pre-approval decisions/ADRs, ACs not latest-Met, open defects, undischarged risks), advisory rules, `human_required` from declared `execution_gates` (prose never machine-evaluated). The same blocking rules guard the phase/slice `Implemented` transition in `entity_upsert` (item-level `"force": true` after explicit operator confirmation; the server appends the FORCED `PE-` audit row itself) |
...
| `handoff_emit(target_dir, subdir?, force?)` | mutate | v3 (plan 027): **pure target wiring** — `.mcp.json` (omitted on plugin-hosted servers) + the marker-managed `CLAUDE.md` operating note (`tamheed:note v2`, with the recording-obligations table; identical=unchanged, changed=updated, hand-edited=diverged unless `force`, v1 notes warned never touched). NO prompt copies — `<package>/prompts/` is the single source (`subdir` refused; leftover `handoff/prm-*.md` warned). Requires ≥1 project-authored prompt file; G-INJECT + the stale scan run over every package prompt file; refreshes the stock library; reports `stale_references` AND `restated_content` |
| `package_migrate(source_dir, name?, confirm?, allow_zero?, patch?, status_map?)` | staged | Migrate a conformant v1 package (preview, then `confirm`); `status_map` confirms/overrides the grouped `status_coerced` proposals; post-flight carries the **fidelity ledgers** (truncations, column_starvation, field_mapping, execution-state note — column-level checks row-level validation cannot see); emits the prompt library and leaves the package open on success |

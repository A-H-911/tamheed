...
| `migrate` | Convert a v2/v3 store to v4 IN PLACE | `package_migrate(name)` (staged: preview = full rewrite report, nothing written → operator backs up → `confirm=true`; old files kept in `data-v3-backup/`). A v4 store missing newer baseline entity types takes the staged **registry-sync** path instead: preview reports `mode: "registry-sync"` + `entity_types_added` + `columns_added` (files that re-serialize because their tables gained columns), confirm appends the registry rows (pure registry append — no backup taken); an up-to-date v4 store still refuses. v1 Keystone trees are no longer ingested — the two-step escape route is documented in the repo's docs | migration report + refreshed `<package>/prompts/` library |

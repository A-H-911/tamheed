...
- **`readiness_check(scope, id?)`** (plan 027) is the semantic layer ABOVE these gates: at a
  ...
  rules (twenty-three at package scope, three of them only when the package has waivers, feedback or skill rows — from overdue open questions through `lessons-confirmed`, `feedback-unanswered`, `deferred-work-carried` (an Activated deferred row no open wbs-item `carries`; v5), `handoff-current` (work journalled after the latest `handoff` entry, or with none at all — the resume state is behind the record; v5.1 — its `population` is every work-done/transition entry, the engine's own `system:` transitions included, and its entities are the uncovered ones; v5.2) and `lessons-stranded` (Promoted lessons whose retired skill row has neither `superseded_by` nor `upstreamed_to`; v5.1 — its `population` counts the Promoted lessons, `unit: promoted lessons`; v5.2), and

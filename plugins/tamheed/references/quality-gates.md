...
| G-COMPLETE | Critical | No placeholder text (TODO/TBD/FIXME/`{{…}}`/`<placeholder>`) in any entity's text columns. Code spans are stripped first (the retired v1 gate's `strip_code` semantics, D-017-4) and `custom_attributes` is exempt — provenance preserved verbatim, graded by G-INJECT at emission instead. v4: `[NEEDS-CLARIFICATION: OQ-NNN]` markers are validated here — legal only while the cited OQ exists and is unresolved; a marker with no id, a dangling id, or a resolved cite fails. |

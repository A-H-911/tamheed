# Artifact Catalog — the entity families and their rules (tamheed v4.13.0)
...
| feedback | `FB-` | Continuous | Upstream feedback and local tools, on the operator's word (v4.11, plan 087). `kind` missing-capability\|defect\|doc-error\|question\|local-tool; `detail` (what was needed), `workaround` (what the agent did instead — the column that catches side tools), `tool_or_rule`, `plugin_version`; a `local-tool` row names its `tool_path`, has no draft stage (refused at INSERT — or at a later `kind` change — without `operator_confirm`; born Confirmed) and obeys a two-clause rule: it writes nothing tool-owned; if it reads the STORE, it reads `exports/` only. Lifecycle Proposed → Confirmed (`operator_confirm` + `confirmed_by`; the server journals the typed event) → Reported (it left the package via `entity_export("feedback")`, into the project's findings) → Resolved (`resolved_in`) / Rejected; withdrawing a Confirmed row needs the word too. `handoff_emit` names Proposed and unexported Confirmed rows every emission; no row text reaches an emitted file. Exempt from `prose-ids-resolve` (feedback quotes broken ids by nature) |

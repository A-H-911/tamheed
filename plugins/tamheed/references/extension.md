...
| New scenario prompt (emitted library) | Add a `.md` beside the existing five; `{package}` is the only substitution; it is emitted into `<package>/prompts/` by migrate/adopt/handoff automatically. | `../prompts/` |

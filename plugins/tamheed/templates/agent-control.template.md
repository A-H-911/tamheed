...
| you need a function tamheed lacks, meet a defect or a wrong doc in it, or would build a script over the package | a `feedback` row (`FB-`, born Proposed) FIRST — never a side tool: a local tool exists only as a `local-tool` row the OPERATOR confirmed, reads `exports/` only and writes nowhere tool-owned; `handoff_emit` names every row until it has left the package |
